import { FacebookOutlined, GithubOutlined, GoogleOutlined } from "@ant-design/icons";
import { faMicrosoft } from "@fortawesome/free-brands-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { TextField } from "@mui/material";
import { Button } from "antd";
import { Link } from "react-router-dom";





function Auth() {
    return (
        <div className="bg-gray-100 pt-24">
            <div className="w-full h-full grid place-items-center gap-5 ">
                <div>
                    <h1 className="font-bold text-4xl underline">Welcome Back!</h1>
                </div>
                <div className="flex flex-col gap-2">
                    <TextField id="outlined-basic" label="Email Address" variant="outlined" type="email" className="w-72" />
                    <TextField id="outlined-basic" label="Password" variant="outlined" type="password" className="w-72" />
                    <button className="font-semibold text-lg w-72 bg-green-500 text-white rounded-lg h-10">Log In </button>
                    <p className="flex justify-evenly font-bold"> Or Don't Have An Account ?<Link to={'/Auth/SignUP'} className="text-green-600">Sign UP</Link> </p>
                </div>

                <div className="text-center font-bold">
                    OR
                    <br />
                    __________________________________________
                </div>
                <div className="flex flex-col gap-2 p-5">
                    <Button icon={<GoogleOutlined />} className="font-semibold text-lg w-72 h-10 bg-yellow-400">Log In with Google</Button>
                    <Button icon={<FacebookOutlined />} className="font-semibold text-lg w-72 h-10 bg-blue-600">Log In with Facebook</Button>
                    <Button className="font-semibold text-lg w-72 bg-black h-10 text-white"><FontAwesomeIcon icon={faMicrosoft} />  Log In with MicroSoft</Button>
                </div>
            </div>
        </div>
    )
}


export default Auth;